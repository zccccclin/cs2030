...
class Expr<T> {
    ...
    public Expr<T> op(Operator<T> operator, T otherValue) {
        Expr<T> result = op(operator, Expr.<T>of(otherValue));
        return result;
    }
    
    public Expr<T> op(Operator<T> operator, Expr<T> otherOperatedValue) {
        Supplier<Optional<Expr<T>>> otherOperatoredValue = () -> 
            Optional.of(Expr.of(otherOperatedValue.evaluate()));
        Expr<T> result = this.operatorStream.map(x -> {
            if (x.compareTo(operator) <= 0) {
                return new Expr<T>(
                    () -> this.evaluate(),
                    Optional.of(operator),
                    otherOperatoredValue);
            } else {
                return new Expr<T>(
                    this.value, 
                    this.operatorStream,
                    () -> this.otherValue.get().map(y -> y.op(operator, otherOperatedValue)));
            }}).orElse(new Expr<T>(this.value, Optional.of(operator), otherOperatoredValue));
        return result;
    }

    public T evaluate() {
        return this.operatorStream.map(
                x -> x.apply(this.value.get(), this.otherValue.get().map(
                    y -> y.evaluate()).orElseThrow()))
            .orElseGet(this.value);
    }
